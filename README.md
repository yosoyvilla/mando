...
- **Adding people:** an admin sees a **Users** section in the web interface (hidden for everyone else) to create, list, and remove accounts. Creating a user takes only an email; the hub generates a one-time temporary password and shows it to the admin **once** — copy it and pass it to the new user, who signs in with it. It is never shown again; the new user should then change it (see below).
- **Changing your password:** any user can change their own password from **Settings > Account** (enter the current password, then the new one). Changing your password signs out all your other sessions and keeps the one you changed it from — so if a password ever leaks, changing it immediately cuts off any session opened with the old one.
- **Promoting and demoting admins:** an admin can grant or revoke admin on any user from the **Users** section (the row's **Make admin** / **Remove admin** action). The last remaining admin cannot be demoted, so the instance can never be left with no admin.
...
- **Audit trail:** security-relevant events (logins, pairing approvals, revocations, account deletions, invites, password changes, and role changes) are recorded in an append-only audit log that admins can review. The audit trail is deliberately kept even when a user is deleted.
